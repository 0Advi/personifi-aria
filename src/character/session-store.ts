...
export async function runMigrations(): Promise<void> {
  ...
  // ── lightweight reminders for Alpha tool-calling ─────────────────────────
  await p.query(`
  ...
  `)
  await p.query(`CREATE INDEX IF NOT EXISTS idx_user_reminders_user_status ON user_reminders(user_id, status, created_at DESC)`)
  await p.query(`CREATE INDEX IF NOT EXISTS idx_user_reminders_scheduled ON user_reminders(scheduled_for) WHERE status IN ('pending', 'scheduled')`)
  await p.query(`
    DO $$
    BEGIN
      IF NOT EXISTS (
