...
describe('rejection-memory (Bedrock fallback path)', () => {
    ...
    beforeEach(() => {
        // Reset modules so any cached Bedrock/AWS config from prior tests is cleared
        vi.resetModules()
        // Ensure Bedrock is disabled for these fallback tests
        delete process.env.AWS_ACCESS_KEY_ID
        delete process.env.AWS_SECRET_ACCESS_KEY
        delete process.env.AWS_REGION
    ...
    it('extractRejectionSignals returns empty for neutral messages (no Bedrock, no Groq signals)', async () => {
        const { extractRejectionSignals } = await import('../intelligence/rejection-memory.js')
        // Message with no positive/negative signal keywords — should bypass both Bedrock and Groq
